...
def admm_compute(A_init, ranks, y_ts, pen_l, pen_k=1, rhos=(1.0, 1.0, 1.0), criterion=criterion, iter_sor=10):
    ...
    def iter_fun(inps):
        ...
        U1, U2, U3 = Us
        iter += 1
        G_flattened_mode1 = mode_unfold_p(G)
        ...
        G, _ = minimize_matrix_input(
            lambda G_tensor: loss_for_G(G_tensor=G_tensor, rhos=rhos, Ds=Ds, Vs=Vs, Cs_flattened=Cs_flattened),
            G)
    ...
    A, _, _, _, _, _, _ = jax.lax.while_loop(criterion, iter_fun, (A, A, 0, Us, G))
    Us, G = hosvd(A, ranks)
    return G, A, Us
