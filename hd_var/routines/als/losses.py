...
def lossU1(y_ts, x_ts, U1, U2, U3, G_flattened_mode1):
    ...
    return np.sum(np.vectorize(_lossU1, signature="(n),(m)->()")(y_ts.T, x_ts.T))
...
def constructx(y_ts, order):
    return constructX(y_ts, order).T
...
def lossU2(y_ts, X_ts, U1, U2, U3, G_flattened_mode1):
    ...
    return np.sum(np.vectorize(_lossU2)(y_ts, X_ts))
...
def lossU3(y_ts, X_ts, U1, U2, U3, G_flattened_mode1):
    ...
    return np.sum(np.vectorize(_lossU3)(y_ts, X_ts))
